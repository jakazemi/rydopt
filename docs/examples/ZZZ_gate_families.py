...
if __name__ == "__main__":
    ...
    use_optimal_zzz_params = False
    reoptimize = False
    plot_results = True
    n_phase_params = 12
    for _ in range(1):
        # a symmetric ansatz for the parity phase gate on three atoms with finite interactions
        target_phases = np.linspace(0.0, 1.0, 11) * np.pi
        # target_phases = [0.05 * np.pi, 0.2 * np.pi, 0.4 * np.pi, 0.6 * np.pi, 0.8 * np.pi, np.pi]
        # target_phases = [0, np.pi/2]
        # target_phases = np.linspace(0.2, 1.0, 5) * np.pi
        # sampled_gates = [
        #     ro.gates.ThreeQubitGateIsosceles(
        #         phi=None,
        #         theta=np.pi-phase,
        #         theta_prime=np.pi-phase,
        #         lamb=-2 * (np.pi-phase),
        #         Vnn=32,
        #         Vnnn=32,
        #         decay=0.0001,
        ...
        #     softmax_scale=0,
        # )
        prototype_gate = ro.gates.ThreeQubitGateIsosceles(
            phi=None,
            theta=None,
            theta_prime=None,
            lamb=None,
        ...
        if not run_optimization:
            filename = "results/"
            filename += "ZZZ_family_n12_fid_1.66e-03_20260706_171126.pkl"
        ...
        degrees = [1, 1, 3, 0]  # duration, detuning, laser phase, Rabi amplitude
        # pulse_map = ro.pulses.PolynomialPulseMapWithCustomDuration(
        #     degrees=degrees, duration_map=empirical_cphase_duration
        # )
        ...
        if run_optimization:
            if use_optimal_zzz_params:
                filename = "results/ZZZ_family_n12_fid_1.07e-03_20260706_134352.pkl"
                ...
                min_initial_params = ro.pulses.PulseFamilyParams(
                    ...
                    * degrees[0],
                    np.zeros(degrees[1] + 1),
                    -2 * np.ones((n_phase_params, degrees[2] + 1)),
                    [1.0],
                )
            ...
            opt_result = ro.optimization.multi_start_optimize(
                ...
                num_steps=200,
                num_processes=4,
                min_converged_initializations=4,
